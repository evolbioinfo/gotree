...
func TestClassicalSupport(t *testing.T) {
	tr, e := support.ClassicalFile("data/rand_tree.nw.gz", "data/rand_tree_boot.nw.gz", 1)
	if e != nil {
		t.Error(e)
	}
	...
	tr, e = support.ClassicalFile("data/rand_tree.nw.gz", "data/rand_tree_same.nw.gz", 1)
	if e != nil {
		t.Error(e)
	}
	for _, e := range tr.Edges() {
	...
	tr, e = support.ClassicalFile("data/rand_tree.nw.gz", "data/rand_tree_half_same.nw.gz", 1)
	if e != nil {
		t.Error(e)
	}
	for _, e := range tr.Edges() {
	...
	tr, e = support.ClassicalFile("data/rand_tree.nw.gz", "data/rand_tree_quarter_same.nw.gz", 1)
	if e != nil {
		t.Error(e)
	}
	for _, e := range tr.Edges() {
